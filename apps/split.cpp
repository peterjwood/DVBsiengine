...
int ProcessSection(SISection *Sect,writer *level, unsigned long *lastsectionlen)
{
	...
	if ( Sect != NULL )
	{
		if ( Sect->complete() == 1)
		{
			if(lastsectionlen != NULL)
				*lastsectionlen += Sect->FullSectionLength(); // store len for multi section in a single packet check
			...
			{
				TMPTable = SITable::AllocateTable(Sect,OC);
...
int PacketLoadingTask()
{
	...
	while (((amountread = read(f,basebuffer,(PACKETBUFFERSIZE/basepacketsize)*basepacketsize))>basepacketsize))
	{
		...
		while(bufferpos <= amountread-basepacketsize)
		{
			if (activePIDS[p.pid])
			{
				if ( p.new_section &&  p.payload_start && (p.lastsectionlen < p.datalength()))
				{
					if ((p.lastsectionlen != 0)  && (p.payload()[p.lastsectionlen] != 0xFF))
					{
						p.new_section = false;
					}
					else
			...
			if (p.lastsectionlen && (Sect[p.pid] == NULL))
			{
				Sect[p.pid] = new SISection(p,p.lastsectionlen);
			}
			else if (p.payload_start && p.new_section && (Sect[p.pid] == NULL))
			{
				Sect[p.pid] = new SISection(p);
			}
			else if (p.payload_start && p.new_section)
			{
				if (Sect[p.pid]->CheckPESSection() && (Sect[p.pid]->SectionLength() == 65535))
				{
					Sect[p.pid]->CompletePES();
					ProcessSection(Sect[p.pid],gettingpids?NULL:level1,NULL);
				}
				else
					delete Sect[p.pid];
			...
			if (Sect[p.pid] != NULL)
			{
				...
				{
					int processval;
					if ((processval = ProcessSection(Sect[p.pid],gettingpids?NULL:level1,&p.lastsectionlen)) == -1)
					{
						delete Sect[p.pid];
					}
					else if (processval == 1)
					{
						lseek(f,basestartpos,0);
...
int SectionLoadingTask()
{
	...
	while (((amountread = read(f,basebuffer,(PACKETBUFFERSIZE/basepacketsize)*basepacketsize))>basepacketsize))
	{
		while(bufferpos <= amountread-basepacketsize)
		{
			...
			{
				if (ProcessSection(CurrentSect,level1,NULL) == -1)
				{
					delete CurrentSect;
				}
